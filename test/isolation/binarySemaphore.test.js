...
import tap from 'tap';
import { BinarySemaphore } from '../../src/isolation/binarySemaphore.js';  // Шлях до вашого BinarySemaphore
...
tap.test('BinarySemaphore test with workers', (t) => {
  if (isMainThread) {
    const buffer = new SharedArrayBuffer(14);
    const semaphore = new BinarySemaphore(buffer, 0);

    t.ok(semaphore, 'BinarySemaphore should be initialized');
    t.same(semaphore.lock, new Int32Array(buffer, 0, 1), 'BinarySemaphore lock should be initialized correctly');
    ...
    const semaphore = new BinarySemaphore(workerData, 0);
    ...
    try {
      ...
      const result = [threadId, semaphore.lock[0], Array.from(array)];
